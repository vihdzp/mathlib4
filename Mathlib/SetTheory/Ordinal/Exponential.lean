...
@[simp, norm_cast]
theorem natCast_pow (m : ℕ) : ∀ n : ℕ, ↑(m ^ n : ℕ) = (m : Ordinal) ^ (n : Ordinal)
  | 0 => by simp
  | n + 1 => by
    rw [pow_succ, natCast_mul, natCast_pow m n, Nat.cast_succ, add_one_eq_succ, opow_succ]

@[deprecated (since := "2026-01-31")]
alias natCast_opow := natCast_pow
