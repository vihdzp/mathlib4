...
instance [Nonempty α] [NoMaxOrder α] : Infinite α := by
  rw [infinite_iff, ← cof_eq_cardinalMk]
  exact aleph0_le_cof

variable (α) in
theorem _root_.IsRegularCardinalOrder.subsingleton_or_noMaxOrder :
    Subsingleton α ∨ NoMaxOrder α := by
  apply (le_or_gt (cof α) 1).imp
  · simp [le_one_iff_subsingleton]
  · rw [← noTopOrder_iff_noMaxOrder]
    exact fun h ↦ cof_ne_one_iff.1 h.ne'
